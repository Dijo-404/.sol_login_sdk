        ...
        <nav className="hidden md:flex items-center gap-1">
          {NAV.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `px-4 py-1.5 rounded-full text-[13px] font-mono transition-all ${isActive ? "text-white bg-white/[0.06] border border-white/10" : "text-slate-400 hover:text-white border border-transparent"}`
              }
            >
              {item.label}
            </NavLink>
          ...
          <div className="px-6 py-5 flex flex-col gap-2">
            {NAV.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
                className="px-3 py-2.5 rounded-lg text-sm font-mono text-slate-300 hover:text-white hover:bg-white/[0.04]"
              >
                {item.label}
              </NavLink>
            ))}
