...
const IdentityBadge = ({ identity }) => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className="group flex items-center gap-2 pl-1 pr-3 py-1 rounded-full bg-white/[0.04] border border-white/10 hover:bg-white/[0.07] hover:border-white/20 transition-all outline-none"
      ...
      >
        <div className="relative">
          {identity.avatar ? (
            ...
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-sol-purple to-sol-teal ring-1 ring-white/10 flex items-center justify-center text-[10px] font-mono text-white">
              {handle[0]}
            </div>
          )}
      ...
      <DropdownMenuContent
        align="end"
        className="bg-navy-900/95 backdrop-blur-xl border-white/10 text-white min-w-[220px]"
      >
        ...
        <DropdownMenuItem
          ...
          className="text-slate-300 focus:text-white focus:bg-white/[0.06] cursor-pointer"
          data-testid="identity-menu-dashboard"
        >
          <Layers size={14} className="mr-2" /> Dashboard
        </DropdownMenuItem>
        {identity.domain && (
          <DropdownMenuItem
            onClick={() => navigate(`/${identity.domain}`)}
            className="text-slate-300 focus:text-white focus:bg-white/[0.06] cursor-pointer"
            data-testid="identity-menu-profile"
          >
        ...
        <DropdownMenuItem
          ...
          className="text-slate-300 focus:text-white focus:bg-white/[0.06] cursor-pointer"
          data-testid="identity-menu-logout"
        >
