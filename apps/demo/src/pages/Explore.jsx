...
const Explore = () => {
  ...
  const result = filter === "all" ? identities : identities.filter((i) => {
    ...
    <div className="relative pt-28 pb-20" data-testid="explore-page">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <div className="mono-label mb-3 flex items-center gap-2"><TrendingUp size={11} className="text-sol-teal" /> Leaderboard</div>
            <h1 className="font-display text-4xl md:text-5xl tracking-tight font-medium">Top <span className="text-gradient-sol">.sol</span> identities</h1>
            <p className="mt-3 text-slate-400 max-w-md">Ranked by aggregate reputation across DeFi, governance, NFT, social, and domain age — resolved live from Solana {client.network}.</p>
          </div>
          <div className="flex flex-wrap gap-1 p-1 rounded-full bg-white/[0.03] border border-white/10" data-testid="explore-filters">
            {tiers.map((t) => (
              <button key={t.id} onClick={() => setFilter(t.id)}
                ...
                data-testid={`filter-${t.id}`}>{t.label}</button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          ...
          <>
            {/* Podium */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12" data-testid="podium">
              {result.slice(0, 3).map((id, i) => (
                <motion.div key={id.wallet} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                  transition={{ duration: 0.6, delay: i * 0.08 }} className="relative glass-card glass-card-hover noise overflow-hidden" data-testid={`podium-${i + 1}`}>
                  <div className="absolute -top-32 -right-32 w-64 h-64 rounded-full blur-3xl pointer-events-none opacity-40"
                    style={{ background: i === 0 ? "#9945FF" : i === 1 ? "#14F195" : "#00C2FF" }} />
                  <Link to={`/${id.domain || id.wallet}`} className="block relative p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div className="font-display text-5xl font-light text-white/15">#{i + 1}</div>
                      {id.reputation && <ReputationMeter score={id.reputation.total} size={70} animated={false} />}
                    </div>
                    <div className="flex items-center gap-3">
                      {id.avatar ? (
                        <img src={id.avatar} alt="" className="w-12 h-12 rounded-xl object-cover ring-1 ring-white/10" />
                      ) : (
            ...
            <div className="glass-card overflow-hidden">
              <div className="hidden md:grid grid-cols-12 gap-4 px-6 py-3 border-b border-white/5 mono-label">
                <div className="col-span-1">Rank</div><div className="col-span-4">Identity</div><div className="col-span-2">Score</div>
                <div className="col-span-3">DeFi · Gov · NFT</div><div className="col-span-2 text-right">Credentials</div>
              </div>
              <div className="divide-y divide-white/5">
                {result.map((id, i) => (
                  <motion.div key={id.wallet} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
                    transition={{ duration: 0.4, delay: i * 0.04 }} className="group" data-testid={`leaderboard-row-${i + 1}`}>
                    <Link to={`/${id.domain || id.wallet}`} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center px-6 py-4 hover:bg-white/[0.02] transition-all">
                      ...
                      <div className="md:col-span-3 flex items-center gap-1.5">
                        {[id.reputation?.breakdown?.defi || 0, id.reputation?.breakdown?.governance || 0, id.reputation?.breakdown?.nft || 0].map((v, j) => (
                          <div key={j} className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                            <div className="h-full rounded-full" style={{ width: `${v * 100}%`, background: j === 0 ? "#9945FF" : j === 1 ? "#00C2FF" : "#14F195" }} />
                          </div>
                        ))}
                      </div>
                      <div className="md:col-span-2 md:text-right">
                        <span className="inline-flex items-center gap-1.5 font-mono text-[11px] text-sol-teal">
                          <span className="w-1.5 h-1.5 rounded-full bg-sol-teal" />{id.credentials?.length || 0} held
                        </span>
                      </div>
                    </Link>
                  </motion.div>
            ...
            <p className="mt-8 text-center text-xs font-mono text-slate-500">Reputation computed live from Solana {client.network} · SNS Protocol</p>
          </>
        )}
      </div>
    </div>
