  ...
  try {
    ...
    for (const session of sessions) {
      try {
        ...
        if (session.domain) {
          try { identity = await resolveIdentity(session.domain); } catch {}
        }

        if (!identity) {
          identity = {
            wallet: session.wallet, domain: session.domain || null,
        ...
        try { identity.reputation = await computeReputation(session.wallet); } catch {}
